...
class ProductManager {
    // id unico para los productos 
    static ultId = 0;

    constructor(path) {
        this.products = [];
        this.path = path; // ruta para crear el archivo
    }
    ...
    async addProduct(objeto) {
        //desestructuracion 
        let { title, description, price, img, code, stock } = objeto;
    ...
    async leerFS(){
        try{
            ...
            console.log("Error, on se pudo leer", e)
        }
    }

    async guardarFS(arrayProductos) {
        try {
            //                      (ruta, archivo preparado)
    ...
    async actualizarFS (id, productoActualizado) {
        try {
            const arrayProductos = await this.leerArchivo();
